...
class TaskGateway
{
    ...
    // Get details from database by USER ID
    ...
    public function updateForUser(int $user_id, string $id, array $data): int
    {
        ...
        if (empty($fields)) {
            ...
            $set = array_map(function ($values) {
                return "$values = :$values";
            }, array_keys($fields));

            $imploded_sets = implode(",", $set);
            $sql = "UPDATE task SET $imploded_sets WHERE id = :id AND user_id = :user_id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindValue(":id", $id, PDO::PARAM_INT);
            $stmt->bindValue(":user_id", $user_id, PDO::PARAM_INT);
